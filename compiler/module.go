...
func LoadModule(raw []byte, pool *pgx.ConnPool) (*Module, error) {
	...
	for _, sec := range m.Customs {
		// Check for the custom section generated by LLVM (8), containing the function names
		if sec.Name == "name" {
			r := bytes.NewReader(sec.RawSection.Bytes)
			for {
				ty, err := leb128.ReadVarUint32(r)
				if err != nil || (ty != 1 && ty != 4) {
					break
				}

				// If type is 4, then the type byte and "name" string need to be skipped.  Everything
				// after that matches the type 1 data so can be parsed by the same code
				if ty == 4 {
					b := make([]byte, 5)
				...
				// Length of the data in this section
				...
				for {
					...
					for i := 0; i < int(count); i++ {
						...
						//   * The length of the name in bytes. eg 0x0A for "wasm_stuff"
